...
- `core/`: 버튼, 탭, disclosure 상태와 의미 토큰을 소유한다. PPT 모델이나 canvas 구현을 import하지 않는다.
- `shell/`: 상단 툴바, 슬라이드 레일, 인스펙터 배치와 점진 공개 규칙을 소유한다.
...
3. 일시적인 툴바 표면은 독립 boolean 대신 `useExclusiveDisclosure`에 추가한다.
